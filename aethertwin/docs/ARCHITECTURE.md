...
A clean close checkpoints, writes `cleanShutdown=true`, truncates WAL, closes SQLite, then removes the held lock. Desktop window-close and process-exit events call `close_all`; registry locks are not held while individual sessions close, successful sessions are removed, failed sessions remain available for retry, and a close failure prevents exit while showing only the sanitized native message and log reference.
