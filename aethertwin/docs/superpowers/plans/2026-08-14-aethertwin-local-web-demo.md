...
- [x] **Step 1: Obtain current explicit approval**
...
- [x] **Step 2: Build the dedicated mode**
...
- [x] **Step 3: Start the localhost server**
...
- [x] **Step 4: Inspect the real browser experience**
...
- [x] **Step 5: Record runtime evidence and remove generated output**
...
- [x] **Step 6: Commit runtime acceptance**
...
**Actual Task 6 evidence (2026-08-14):** Explicit approval covered build, the strict localhost server, headed browser inspection, and real WebGL interaction; screenshot capture remained unapproved and was not run. The final build exited 0 after transforming 3,215 modules, and `dist/index.html` referenced only relative assets. Vite bound `http://127.0.0.1:4173` without a fallback port and remains running for the requested preview. Headed Chromium passed all eleven checks above. The first forced context loss automatically reconstructed the renderer without a duplicate-root error; the second moved 3D to `disabled` and preserved a selectable, floor-synchronized 2D canvas. The browser recorded zero console errors, 181 loopback/same-origin Blob requests, and empty localStorage, sessionStorage, and cookies. Final source evidence passed lint, typecheck across 14/15 workspace projects, Node 53/53, and Vitest 82 files / 1,547 tests. The accepted evidence is machine/browser specific and does not claim cross-device visual correctness or performance.
