...
- [x] **Obtain explicit approval** for `pnpm.cmd package:web-demo:win-x64`,
...
- [x] **Build and verify ZIP placement.** Run the exact root packaging command,
...
- [x] **Run with a restricted PATH.** Launch from the extracted directory with
...
- [x] **Perform browser acceptance in Edge and Chrome.** Confirm the current
  ...
  Blob/data-only; and no remote, `file://`, Player, publish, native-dialog, or
  browser-persistence surface appears.
- [x] **Verify shutdown and integrity.** Close the console and prove the
...
- [x] **Clean up safely.** Remove only the explicitly resolved test extraction
...
- [x] **Commit runtime evidence only if accepted.** If every approved runtime
...
### Task 4 observed evidence

The approved command produced a ZIP from source commit
`6ec8d499425f7578902fc18799839a00ee9e4bd9` with SHA-256
`AC144D56A47EC23C169B0616E7FF6A7596D2302C3CCADD07A0DE331B657E0F84`.
The fixed six-entry root, 26 payload checksums, relative references,
Chinese-and-space extraction, restricted-PATH runtime, preferred and fallback
loopback ports, Edge/Chrome product checks, empty browser storage,
loopback/Blob/data-only network boundary, WebGL2 fallback, and shutdown all
passed. Final source gates passed lint, typecheck, Node 73/74 with one
privileged-Windows symlink skip, Vitest 82 files / 1,547 tests, Rust
fmt/check/18 host tests, and diff check. Generated package, staging, `dist/`,
and verified extraction directories were then safely removed.
