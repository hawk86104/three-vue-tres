...
AetherTwin is a local-first, desktop-first digital-twin authoring product with exactly two immutable project profiles: `showroom` and `market`. M0 through M2.4 are accepted and closed. M2.5 Tasks 1-19 are implemented and independently reviewed; Task 20 is the only remaining final non-build closure gate. Studio opens directly in the 2D editor. Showroom can switch to synchronized 3D or a fixed 50/50 split and export a project-bound PNG, while Market remains 2D-only. Player is still a deferred, intentionally noninteractive boundary.
...
M2.5 Task 18's final vertical gate passed 13/13 focused Vitest tests, three TypeScript checks, media-export 12/12, project-io 2/2, desktop-host 1/1, four-crate all-targets Cargo check, rustfmt, and diff check. The canonical Showroom Demo is offline and deterministic. Full evidence and exclusions are in `docs/M2_REPORT.md`. Task 19's policy gate passed 45/45 and its independent documentation re-review returned Pass / Approved / Ready Yes with no findings. Task 20 is now the only remaining M2 closure gate.
