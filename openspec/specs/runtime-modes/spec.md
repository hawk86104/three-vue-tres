...
### Requirement: 预览菜单数据加载条件

预览相关菜单数据的远程加载 SHALL 受运行模式控制。预览运行时初始化仅应在预览相关模式或显式线上 API 开关开启时加载菜单设置；预览中心的线上插件菜单补充在本地单插件模式下 SHALL 默认关闭，除非显式开启线上 API。

#### Scenario: 本地预览模式加载菜单设置

- GIVEN 当前处于开发环境
- AND `FES_APP_PLUGINS=true`
- WHEN 应用执行启动前初始化
- THEN 系统 SHALL 调用预览菜单设置加载逻辑

#### Scenario: 普通项目模式不主动加载预览菜单设置

- GIVEN 当前环境中 `FES_APP_PLUGINS=false`
- AND 未开启 `FES_APP_ONLINE_API`
- WHEN 应用执行启动前初始化
- THEN 系统 SHALL 不因预览机制而主动加载远程菜单设置

#### Scenario: 单插件模式默认不做线上预览菜单补充
